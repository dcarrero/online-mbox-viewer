...
function build(max: number): Record<string, ViewerCopy> {
  return {
    en: {
      ...
      desktopBody:
        "This web viewer is for a quick look at small archives. For files of any size, search, labels and EML support, get the native desktop app for Mac.",
      metaTitle: "Free Online MBOX Viewer — open MBOX files in your browser",
      metaDescription: `Open and read MBOX files online, free and private. Everything runs in your browser — nothing is uploaded. Up to ${max} MB. For larger files, get Mbox Viewer for Mac.`,
    ...
    es: {
      ...
      desktopBody:
        "Este visor web es para echar un vistazo rápido a archivos pequeños. Para ficheros de cualquier tamaño, búsqueda, etiquetas y soporte EML, descarga la app nativa de escritorio para Mac.",
      metaTitle: "Visor de MBOX online gratis — abre ficheros MBOX en tu navegador",
      metaDescription: `Abre y lee ficheros MBOX online, gratis y privado. Todo se procesa en tu navegador — no se sube nada. Hasta ${max} MB. Para ficheros más grandes, descarga Mbox Viewer para Mac.`,
    ...
    de: {
      ...
      desktopBody:
        "Dieser Web-Viewer ist für einen schnellen Blick auf kleine Archive gedacht. Für Dateien beliebiger Größe, Suche, Labels und EML-Unterstützung hol dir die native Desktop-App für Mac.",
      metaTitle: "Kostenloser Online-MBOX-Viewer — MBOX-Dateien im Browser öffnen",
      metaDescription: `Öffne und lies MBOX-Dateien online, kostenlos und privat. Alles läuft in deinem Browser — nichts wird hochgeladen. Bis zu ${max} MB. Für größere Dateien hol dir Mbox Viewer für Mac.`,
    ...
    fr: {
      ...
      desktopBody:
        "Cette visionneuse web sert à consulter rapidement de petites archives. Pour des fichiers de toute taille, la recherche, les libellés et la prise en charge EML, téléchargez l'application de bureau native pour Mac.",
      metaTitle: "Visionneuse MBOX en ligne gratuite — ouvrez vos fichiers MBOX dans le navigateur",
      metaDescription: `Ouvrez et lisez vos fichiers MBOX en ligne, gratuitement et en privé. Tout s'exécute dans votre navigateur — rien n'est téléversé. Jusqu'à ${max} Mo. Pour les fichiers plus volumineux, téléchargez Mbox Viewer pour Mac.`,
    ...
    "pt-br": {
      ...
      desktopBody:
        "Este visualizador web é para dar uma olhada rápida em arquivos pequenos. Para arquivos de qualquer tamanho, busca, marcadores e suporte a EML, baixe o app nativo de desktop para Mac.",
      metaTitle: "Visualizador de MBOX online grátis — abra arquivos MBOX no navegador",
      metaDescription: `Abra e leia arquivos MBOX online, grátis e com privacidade. Tudo roda no seu navegador — nada é enviado. Até ${max} MB. Para arquivos maiores, baixe o Mbox Viewer para Mac.`,
    ...
    ja: {
      ...
      desktopBody:
        "このウェブビューアは小さなアーカイブをすばやく確認するためのものです。あらゆるサイズのファイル、検索、ラベル、EML対応には、Mac向けネイティブのデスクトップアプリをご利用ください。",
      metaTitle: "無料オンラインMBOXビューア — ブラウザでMBOXファイルを開く",
      metaDescription: `MBOXファイルをオンラインで無料かつプライベートに開いて読めます。すべてブラウザ内で動作し、何もアップロードされません。最大${max}MBまで。大きなファイルにはMac版Mbox Viewerをご利用ください。`,
      loading: "メッセージを読み込み中…",
    ...
    it: {
      ...
      desktopBody:
        "Questo visualizzatore web serve a dare un'occhiata veloce a piccoli archivi. Per file di qualsiasi dimensione, ricerca, etichette e supporto EML, scarica l'app desktop nativa per Mac.",
      metaTitle: "Visualizzatore MBOX online gratuito — apri i file MBOX nel browser",
      metaDescription: `Apri e leggi i file MBOX online, gratis e in privato. Tutto funziona nel tuo browser — niente viene caricato. Fino a ${max} MB. Per file più grandi, scarica Mbox Viewer per Mac.`,
    ...
    nl: {
      ...
      desktopBody:
        "Deze webviewer is bedoeld om snel kleine archieven te bekijken. Voor bestanden van elke grootte, zoeken, labels en EML-ondersteuning download je de native desktop-app voor Mac.",
      metaTitle: "Gratis online MBOX-viewer — open MBOX-bestanden in je browser",
      metaDescription: `Open en lees MBOX-bestanden online, gratis en privé. Alles draait in je browser — er wordt niets geüpload. Tot ${max} MB. Voor grotere bestanden download je Mbox Viewer voor Mac.`,
    ...
    ko: {
      ...
      desktopBody:
        "이 웹 뷰어는 작은 아카이브를 빠르게 살펴보기 위한 것입니다. 모든 크기의 파일, 검색, 라벨, EML 지원이 필요하면 Mac용 네이티브 데스크톱 앱을 받으세요.",
      metaTitle: "무료 온라인 MBOX 뷰어 — 브라우저에서 MBOX 파일 열기",
      metaDescription: `MBOX 파일을 온라인에서 무료로 비공개로 열고 읽으세요. 모든 작업이 브라우저에서 실행되며 아무것도 업로드되지 않습니다. 최대 ${max}MB. 더 큰 파일은 Mac용 Mbox Viewer를 받으세요.`,
      loading: "메시지 불러오는 중…",
    ...
    pl: {
      ...
      desktopBody:
        "Ta przeglądarka internetowa służy do szybkiego podglądu małych archiwów. Aby obsłużyć pliki dowolnego rozmiaru, wyszukiwanie, etykiety i format EML, pobierz natywną aplikację desktopową na Maca.",
      metaTitle: "Darmowa przeglądarka MBOX online — otwórz pliki MBOX w przeglądarce",
      metaDescription: `Otwieraj i czytaj pliki MBOX online, za darmo i prywatnie. Wszystko działa w Twojej przeglądarce — nic nie jest wysyłane. Do ${max} MB. W przypadku większych plików pobierz Mbox Viewer na Maca.`,
